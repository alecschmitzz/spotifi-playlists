...
import { buildMakePlaylist } from './playlist';
...
describe('buildMakePlaylist', () => {
    ...
    const makePlaylist = buildMakePlaylist({
        Id: {
            makeId: makeIdMock,
    ...
    it('creates a valid Playlist object', () => {
        const validPlaylistData = {
            title: 'Valid Title',
            description: 'Valid Description',
        };

        const playlist = makePlaylist(validPlaylistData);

        expect(playlist.getTitle()).toBe('Valid Title');
        expect(playlist.getDescription()).toBe('Valid Description');
        expect(playlist.getId()).toBe('mocked-id');
        expect(typeof playlist.getHash()).toBe('string');
    ...
    it('throws an error for an invalid id', () => {
        ...
        expect(() => makePlaylist(invalidPlaylistData)).toThrow('Playlist must have a valid id.');
    });
